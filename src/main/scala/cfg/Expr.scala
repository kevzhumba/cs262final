...
class ExprSerializer
    extends CustomSerializer[Expr](format =>
      (
        {
          case JObject(JField("value", JInt(i)) :: Nil) =>
            IntConstant(i.intValue)
          case JObject(JField("vars", JArray(x)) :: Nil) =>
            Var(x.map(_.asInstanceOf[JInt].num.intValue))
          case JObject(
            ...
            BinExpr(
              ...
              right.extract(
                format,
                Manifest.classType(classOf[Expr])
              )
            )
          case JObject(
                JField("op", JInt(id)) :: JField("operand", operand) :: Nil
              ) =>
            UnExpr(
              UnOps(id.intValue),
              operand.extract(format, Manifest.classType(classOf[Expr]))
            )
          case JObject(
        ...
        },
        {
          ...
          case v: Var =>
            JObject(JField("vars", JArray(v.vars.map(JInt(_)))) :: Nil)
          case b: BinExpr =>
            JObject(
          ...
          case r: ReadField =>
            JObject(
              JField(
                "declaringClass",
                Extraction.decompose(r.declaringClass)(format)
              ) ::
                JField("name", JString(r.name)) ::
                JField(
                  "fieldType",
                  Extraction.decompose(r.declaredFieldType)(format)
                ) ::
    ...
    expr match {
      ...
      case nvf: tac.NonVirtualFunctionCall[DUVar[ValueInformation]] =>
        NonVirtualFunctionCall(
          ...
          nvf.isInterface,
          nvf.name,
          translate(nvf.receiver),
          nvf.params.map(translate(_))
        )
  ...
  def getVars(expr: Expr): Set[Int] =
    expr match {
      ...
      case VirtualFunctionCall(
        ...
        getVars(receiver) ++ params.flatMap(p => getVars(p)).toSet
      case StaticFunctionCall(declaringClass, isInterface, name, params) =>
        params.flatMap(p => getVars(p)).toSet
