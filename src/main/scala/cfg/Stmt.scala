...
class FieldTypeSerializer
    extends CustomSerializer[FieldType](format =>
      (
        {
          ...
          case o =>
            o.extract(DefaultFormats, Manifest.classType(classOf[ObjectType]))
        },
        {
...
class StmtSerializer
    extends CustomSerializer[Stmt](format =>
      (
        {
          case JObject(JField("targetVar", e1) :: JField("expr", e2) :: Nil) =>
            Assignment(
              e1.extract(format, Manifest.classType(classOf[Var])),
              e2.extract(format, Manifest.classType(classOf[Expr]))
            )
          case JObject(
                JField("left", left) :: JField("op", JInt(id)) :: JField(
                  "right",
                  right
                ) :: Nil
              ) =>
            If(
              left.extract(format, Manifest.classType(classOf[Expr])),
              RelationalOps(id.intValue),
              right.extract(format, Manifest.classType(classOf[Expr]))
            )
          case JObject(JField("ret", ret) :: Nil) =>
            ReturnValue(ret.extract(format, Manifest.classType(classOf[Expr])))
          case JObject(
                JField("declaringClass", declaringClass) :: JField(
                  "isInterface",
                  JBool(isInterface)
                ) :: JField("name", JString(name)) :: JField(
                  "params",
                  JArray(p)
                ) :: Nil
              ) =>
            StaticMethodCall(
              declaringClass
                .extract(format, Manifest.classType(classOf[ObjectType])),
              isInterface,
              name,
              p.map(a =>
                a.extract[Expr](format, Manifest.classType(classOf[Expr]))
              )
            )
          ...
          case JObject(
            ...
            VirtualMethodCall(
              declaringClass.extract(
                format,
                Manifest.classType(classOf[ObjectType])
              ), // This might be wrong
              isInterface,
              name,
        ...
        },
        {
          ...
          case s: StaticMethodCall =>
            JObject(
              JField(
                "declaringClass",
                Extraction.decompose(s.declaringClass)(format)
              ) ::
                ...
                JField(
                  "params",
                  JArray(
                    s.params.map(p => Extraction.decompose(p)(format)).toList
                  )
                ) ::
                Nil
            )
          case v: VirtualMethodCall =>
            JObject(
              JField("virtual", JBool(true)) ::
                JField(
                  "declaringClass",
                  Extraction.decompose(v.declaringClass)(format)
                ) ::
                ...
                JField(
                  "params",
                  JArray(
                    v.params.map(p => Extraction.decompose(p)(format)).toList
                  )
                ) ::
                Nil
            )
          ...
          case p: PutField =>
            JObject(
              JField(
                "declaringClass",
                Extraction.decompose(p.declaringClass)(format)
              ) ::
                JField("name", JString(p.name)) ::
                JField(
                  "fieldType",
                  Extraction.decompose(p.declaredFieldType)(format)
                ) ::
...
object StmtUtil {
  def translateStmt(stmt: tac.Stmt[DUVar[ValueInformation]]): Stmt =
    stmt match {
      case i: tac.If[DUVar[ValueInformation]] =>
        If(
          translate(i.leftExpr),
          RelationalOps(i.condition.id),
          translate(i.rightExpr)
        )
      case goto: tac.Goto  => Goto()
      case ret: tac.Return => Return()
      case a: tac.Assignment[DUVar[ValueInformation]] =>
        Assignment(translate(a.targetVar).asInstanceOf[Var], translate(a.expr))
      case ret: tac.ReturnValue[DUVar[ValueInformation]] =>
        ReturnValue(translate(ret.expr))
      case nop: tac.Nop => Nop()
      case pf: tac.PutField[DUVar[ValueInformation]] =>
        PutField(
          pf.declaringClass,
          pf.name,
          pf.declaredFieldType,
          translate(pf.objRef),
          translate(pf.value)
        )
      case nvm: tac.NonVirtualMethodCall[DUVar[ValueInformation]] =>
        NonVirtualMethodCall(
          ...
          nvm.isInterface,
          nvm.name,
          translate(nvm.receiver),
          nvm.params.map(translate(_))
        )
      case vm: tac.VirtualMethodCall[DUVar[ValueInformation]] =>
        VirtualMethodCall(
          ...
          vm.isInterface,
          vm.name,
          translate(vm.receiver),
          vm.params.map(translate(_))
        )
      case sm: tac.StaticMethodCall[DUVar[ValueInformation]] =>
        StaticMethodCall(
          ...
          sm.isInterface,
          sm.name,
          sm.params.map(translate(_))
        )
      case e: tac.ExprStmt[DUVar[ValueInformation]] =>
        ExprStmt(translate(e.expr))
    }

  def getVars(stmt: Stmt): Set[Int] = stmt match {
    case If(left, relationalOps, right) =>
      ExprUtil.getVars(left) ++ ExprUtil.getVars(right)
    case Assignment(targetVar, expr) =>
      ExprUtil.getVars(targetVar) ++ ExprUtil.getVars(expr)
    case ReturnValue(ret) => ExprUtil.getVars(ret)
    case PutField(declaringClass, name, declaredFieldType, objRef, value) =>
      ExprUtil.getVars(objRef) ++ ExprUtil.getVars(value)
    case NonVirtualMethodCall(
      ...
      ExprUtil
        .getVars(receiver) ++ params.flatMap(p => ExprUtil.getVars(p)).toSet
    ...
    case StaticMethodCall(declaringClass, isInterface, name, params) =>
      params.flatMap(p => ExprUtil.getVars(p)).toSet
...
case class If(left: Expr, relationalOps: RelationalOps.Value, right: Expr)
    extends Stmt
