...
/** Loads a Java project from a file and constructs a Project object.
  *
  * @param f
  *   File object representing the Java project
  * @return
  *   Project object representing the Java project
  */
object JavaLoader {
  ...
  def loadJavaProject(f: File): Project[URL] = {
    implicit val config = BaseConfig
      ...
      .withValue(
        ...
        ConfigValueFactory.fromAnyRef(new util.ArrayList[String]())
      )
    val p = Project(
      JavaClassFileReader().ClassFiles(f) ++ JavaClassFileReader().ClassFiles(
        RTJar
      ),
      Iterable.empty,
